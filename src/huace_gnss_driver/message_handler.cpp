...
Timestamp MessageHandler::timestampHuace(float tow, uint32_t wnc) const
{
  ...
  constexpr uint64_t nsOfGpsStart =
    315964800 * secToNSec;  // GPS week counter starts at 1980-01-06 which is
  ...
  if (current_leap_seconds_ != -128) time_obj -= current_leap_seconds_ * secToNSec;
  // else: warn?
...
void MessageHandler::publish(const std::string & topic, const M & msg)
{
  ...
  if (!settings_->use_gnss_time || (settings_->use_gnss_time && (current_leap_seconds_ != -128))) {
...
void MessageHandler::parseNmea(const std::shared_ptr<Telegram> & telegram)
{
  ...
  std::cout << "parseNmea:" << message << std::endl;
  ...
  std::cout << "body[0]:" << body[0] << ",id:" << id << std::endl;
  ...
  if (it != nmeaMap_.end()) {
    switch (it->second) {
      case 0: {
        ...
        try {
          msg = parser_obj.parseASCII(
            gga_message, settings_->frame_id, settings_->use_gnss_time, telegram->stamp);
        } catch (ParseException & e) {
          node_->log(log_level::DEBUG, "GpggaMsg: " + std::string(e.what()));
          break;
        }
      ...
      case 1: {
        ...
        try {
          msg = parser_obj.parseASCII(
            rmc_message, settings_->frame_id, settings_->use_gnss_time, telegram->stamp);
        } catch (ParseException & e) {
          node_->log(log_level::DEBUG, "GprmcMsg: " + std::string(e.what()));
          break;
        }
      ...
      case 4: {
        ...
        try {
          msg = parser_obj.parseASCII(
            chc_message, settings_->frame_id, settings_->use_gnss_time, node_->getTime());
        } catch (ParseException & e) {
          node_->log(log_level::WARN, "GpchcMsg: " + std::string(e.what()));
          break;
        }
        if (settings_->use_gnss_time) {
      ...
      case 5: {
        ...
        try {
          msg = parser_obj.parseASCII(
            chcx_message, settings_->frame_id, settings_->use_gnss_time, node_->getTime());
        } catch (ParseException & e) {
          node_->log(log_level::WARN, "GpchcxMsg: " + std::string(e.what()));
          break;
        }
        if (settings_->use_gnss_time) {
          Timestamp time_obj = timestampHuace(msg.tow, msg.wnc);
